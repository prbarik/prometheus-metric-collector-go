...
func instrumentedHandler(w http.ResponseWriter, r *http.Request, serviceName string) {
	...
	message := fmt.Sprintf("Prometheus Metrics example response for %s in %f ns\n", serviceName, elapsed)
...
func handler(w http.ResponseWriter, r *http.Request) {
	_, _ = w.Write([]byte("Prometheus Metrics example. Hit the /metrics end point to see Prometheus Exposition metrics..."))
	w.WriteHeader(200)
}
